...
public class BestSplit {
    ...
    static {
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader(new File(BestSplit.class.getResource("testData.txt")
                    .toURI())));
            String s;
            while ((s = br.readLine()) != null) {
                ...
                if (temp.length == 2) {
                    try {
                        EntityData ed = new EntityData(Double.parseDouble(temp[0]), ClassEnum.valueOf(temp[1]
                                .toUpperCase()));
                        dataSet.add(ed);
                    } catch (Exception e) {
                        continue;
